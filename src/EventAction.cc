...
void EventAction::EndOfEventAction(const G4Event* ev)
{
  ...
  if(HCE) {
	...
	if(crystalXforms){
	  //G4double xx, yy;
	  x = (*gammaCollection)[i]->GetPosCrys().getX()/mm;
	  y = (*gammaCollection)[i]->GetPosCrys().getY()/mm;
	...
	if((*gammaCollection)[i]->GetParticleID() == "gamma"){
	  ...
	  if(i > 0 
	    ...
	    interactionType[NMeasured] = 1; // Gamma!
	...
	if(!processed){
	  ...
	  interactionType[NMeasured] = 0; // Electron!
	  trackID[NMeasured]      = (*gammaCollection)[i]->GetTrackID();
	  detNum[NMeasured]       = (*gammaCollection)[i]->GetDetNumb();
	  segNum[NMeasured]       = (*gammaCollection)[i]->GetSegNumb();
	  measuredEdep[NMeasured] = en;
	...
	writeS800(timestamp, 
      ...
      writeDecomp(timestamp, 
		  ...
		  measuredEdep, segmentEdep, interactionType);
...
void EventAction::writeDecomp(long long int ts, 
			      ...
			      G4double e[], G4double se[], G4int type[])
{
  G4int siz;
  GEBDATA gd;
  CRYS_IPS crys_ips[10000];

  G4int evType[10000][16]; 
  ...
  for(G4int i = 0; i < NMeasured; i++){
	...
	if(NCons[j] > 0 && 
	  ...
	  evType[Ndecomp][crys_ips[Ndecomp].num] = type[j];
